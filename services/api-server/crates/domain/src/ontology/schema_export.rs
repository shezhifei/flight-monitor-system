//! Ontology V1 schema 导出契约（契约 §7；交接 Phase 0）。
//!
//! 版本命名决策：
...
//! （契约 §4.1），不属于 schema 导出；schema 只声明 `arguments_schema`。
...
/// schema 中单个动作的导出视图（契约 §4.1 的声明式子集）。
...
/// `/api/v2/ai/ontology/schema` 的稳定导出结构（契约 §7）。
#[derive(Debug, Clone, Serialize)]
...
pub fn build_schema_export(schema: &OntologySchema, exported_at: DateTime<Utc>) -> OntologySchemaExport {
    ...
    for (object_name, object) in &schema.objects {
        for (action_name, action) in &object.actions {
            ...
            actions.insert(
                key.clone(),
                export_action_ref(schema, object_name, action),
            );
    ...
    constraints.sort_by(|a, b| {
        (&a.object_type, &a.action_name, &a.constraint.expression)
            .cmp(&(&b.object_type, &b.action_name, &b.constraint.expression))
    });
    ...
    match value {
        Value::Object(map) => {
            let sorted: BTreeMap<String, Value> = map
                .into_iter()
                .map(|(key, item)| (key, canonical_json(item)))
                .collect();
...
mod tests {
    ...
    fn exported_actions_carry_contract_fields() {
        ...
        for (key, action) in &export.actions {
            ...
            assert!(!action.required_permissions.is_empty(), "{key} must declare permissions");
            assert_eq!(action.arguments_schema["type"], json!("object"), "{key} arguments_schema");
        }
    }
    ...
    fn flight_ops_v1_export_matches_fixture() {
        ...
        let expected_text = std::fs::read_to_string(&path)
            .unwrap_or_else(|err| panic!("missing ontology fixture {} (regenerate with UPDATE_ONTOLOGY_FIXTURE=1): {err}", path.display()));
