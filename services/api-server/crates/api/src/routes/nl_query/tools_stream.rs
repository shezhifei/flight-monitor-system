...
pub(crate) async fn query_natural_language_stream_with_tools(
    ...
    // Task I4: same pinned-task-type validation as the plain stream route.
    ...
    let job = job_service
        .create_job("nl_query_stream_with_tools", Some(&user_id), None, None, None)
        .await
    ...
    let python_resp_res = runtime_client.stream_run_with_tools_raw(&req, &envelope_value).await;
    ...
    match python_resp_res {
        Ok(python_resp) => {
            ...
            spawn_tracked("nl_query_tools_stream", async move {
...
pub(crate) async fn stream_with_tools_gate(
    req: HttpRequest,
    claims: JwtAuth,
    body: web::Json<NLQueryRequest>,
    job_service: web::Data<Arc<AiJobService>>,
    ...
    let tool_streaming_enabled = std::env::var("AI_RUNTIME_ENABLE_TOOL_STREAMING")
    ...
    if !tool_streaming_enabled {
        return Ok(HttpResponse::ServiceUnavailable().json(json!({
            "success": false,
            "data": null,
            "message": "Tool streaming is an alpha feature (P2.4-alpha) and is disabled by default. Set AI_RUNTIME_ENABLE_TOOL_STREAMING=1 to enable.",
            "degraded": true,
            "feature_gate": "AI_RUNTIME_ENABLE_TOOL_STREAMING",
        })));
    }
    query_natural_language_stream_with_tools(
        req,
        claims,
        body,
        job_service,
