//! AI Config V2 management routes
//!
...
const AI_CONFIG_V2_PREFIX: &str = "/api/v2/ai";
const AI_CONFIG_INTERNAL_PREFIX: &str = "/internal/ai/v1";

fn config_target_url(req: &HttpRequest) -> String {
    let suffix = req.path().strip_prefix(AI_CONFIG_V2_PREFIX).unwrap_or_default();
...
/// Register the config-v2 proxy routes RELATIVE to a parent `/api/v2/ai` scope.
///
...
mod route_behavior_tests {
    ...
    fn build_test_app_with_config_v2() -> App<
    ...
    #[actix_web::test]
    async fn v2_config_routes_are_mounted_not_found_without_sidecar() {
        let app = test::init_service(build_test_app_with_config_v2()).await;

        // GET paths that ai_config_v2 owns exclusively (no conflict with ai.rs)
        let get_paths = [
    ...
    #[actix_web::test]
    async fn v2_capabilities_route_does_not_match_entity_get() {
        let app = test::init_service(build_test_app_with_config_v2()).await;
        ...
        assert_ne!(
            ...
            "/entities/{{id}}/capabilities must be handled by ai_config_v2, not ai"
        );
    }

    #[actix_web::test]
    async fn v2_mcp_servers_post_route_mounted() {
        let app = test::init_service(build_test_app_with_config_v2()).await;
    ...
    #[actix_web::test]
    async fn v2_mcp_resource_read_post_route_mounted() {
        let app = test::init_service(build_test_app_with_config_v2()).await;
    ...
    #[actix_web::test]
    async fn v2_skills_bindings_delete_route_mounted() {
        let app = test::init_service(build_test_app_with_config_v2()).await;
    ...
    #[actix_web::test]
    async fn v2_cache_metrics_get_preserves_query() {
        let app = test::init_service(build_test_app_with_config_v2()).await;
    ...
    #[actix_web::test]
    async fn v2_cache_invalidate_post_route_mounted() {
        let app = test::init_service(build_test_app_with_config_v2()).await;
