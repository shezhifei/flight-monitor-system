...
class TestTaskTypePrecedence:
    ...
    async def test_task_type_overrides_keywords_dispatch_ops(self):
        ...
        assert decision.template == "dispatch_ops", \
            "Should use dispatch_ops template from task_type"
        assert decision.confidence == 1.0, \
            "Configuration-based routing should have confidence 1.0"
        assert decision.source == "entity_config", \
            "Source must be entity_config, not keyword_fallback"
    ...
    async def test_task_type_overrides_keywords_anomaly_ops(self):
        ...
        assert decision.template == "anomaly_ops", \
            "Should use anomaly_ops template from task_type"
        assert decision.confidence == 1.0
    ...
    async def test_keyword_fallback_without_task_type(self):
        ...
        assert decision.confidence == 0.7, \
            "Keyword fallback should provide confidence 0.7"
        assert decision.source == "keyword_fallback", \
            "Source should be keyword_fallback when no task_type configured"
    ...
    async def test_default_routing_with_no_intent(self):
        ...
        assert decision.confidence == 0.0, \
            "Default routing should have confidence 0.0"
        assert decision.source == "default", \
            "Source should be default for empty/ambiguous input"
...
class TestToolFilteringByTemplate:
    ...
    async def test_query_ops_filters_to_readonly_tools(self):
        ...
        assert len(filtered_tools) >= len(all_tools), \
            "Should keep all tools if query_ops filter too aggressive"
    ...
    async def test_dispatch_ops_allows_write_actions_proposal_only(self):
        ...
        assert "change_stand" in tool_names, \
            "dispatch_ops should include change_stand tool"
        assert "notify_teams" in tool_names, \
            "dispatch_ops should include notify_teams tool"
    ...
    async def test_general_ops_returns_all_tools(self):
        ...
        assert len(filtered_tools) == len(all_tools), \
            "general_ops should return all tools unchanged"
...
class TestIntentRoutingSafety:
    ...
    async def test_minimum_tool_safety_guard(self):
        ...
        assert len(filtered) == 1, \
            "Invalid templates should return minimal set unchanged"
    ...
    async def test_high_confidence_routing_indicates_config_source(self):
        ...
        assert decision.is_high_confidence(), \
            "Config-based routing should be marked as high confidence"
    ...
    async def test_low_confidence_routes_require_review(self):
        ...
        assert not decision.is_high_confidence(), \
            "Default routing should not be high confidence"
...
class TestEntityPatternMatching:
    ...
    async def test_dispatch_pattern_inference(self):
        ...
        assert decision.source == "entity_config", \
            "Entity pattern should trigger config-based routing"
    ...
    async def test_anomaly_pattern_inference(self):
        ...
        assert decision.template == "anomaly_ops", \
            "Entity pattern should match anomaly template"
...
def test_intent_router_architecture_complete():
    ...
    assert actual_count == expected_count, \
        f"All {expected_count} architecture features must be implemented"
