  ...
  <aside class="editor-properties-panel">
    ...
    <header class="panel-header">
      ...
      <span
        ...
        :class="p.selectedFormTaskConfig.value ? 'editing' : 'active'"
      >
    ...
    <div v-show="activeTab === 'node'" class="panel-tab-content" role="tabpanel">
        ...
        <div class="panel-card">
          ...
          <div class="panel-grid-2">
            <label class="panel-field">
              ...
              <input type="text" class="panel-input mono" :value="p.selectedNotificationRule.value.action" @input="p.updateSelectedNotificationRule({ action: ($event.target as HTMLInputElement).value })">
            </label>
            <label class="panel-field">
      ...
      <template v-else-if="p.selectedTaskId.value && p.selectedNodeType.value === 'wait_receipts'">
        ...
        <div class="panel-card">
          ...
          <label class="panel-field">
            <span class="panel-label">节点标题</span>
            <input type="text" class="panel-input" :value="p.selectedTaskName.value" @input="updateSelectedTaskName($event)">
          </label>
          <p class="panel-hint">
        ...
        <div class="panel-card">
          ...
          <div class="panel-grid-2">
            <label class="panel-field">
              ...
              <input type="text" class="panel-input mono" :value="p.selectedFormTaskConfig.value.templateCode" placeholder="form_template_xxx" @input="updateSelectedTaskConfig({ templateCode: ($event.target as HTMLInputElement).value })">
            </label>
            <label class="panel-field">
              <span class="panel-label">表单代码 Form</span>
              <input type="text" class="panel-input mono" :value="p.selectedFormTaskConfig.value.formCode" placeholder="form_xxx" @input="updateSelectedTaskConfig({ formCode: ($event.target as HTMLInputElement).value })">
            </label>
            <label class="panel-field">
              <span class="panel-label">版本 Version</span>
              <input type="number" min="1" class="panel-input" :value="p.selectedFormTaskConfig.value.version" @input="updateSelectedTaskConfig({ version: Number(($event.target as HTMLInputElement).value) || 1 })">
            </label>
            <label class="panel-field">
              <span class="panel-label">所属部门 Department</span>
              <input type="text" class="panel-input" :value="p.selectedFormTaskConfig.value.department" placeholder="可选" @input="updateSelectedTaskConfig({ department: ($event.target as HTMLInputElement).value })">
            </label>
            <label class="panel-field">
              <span class="panel-label">角色 Roles</span>
              <input type="text" class="panel-input mono" :value="p.selectedTaskRolesText.value" placeholder="dispatcher, supervisor" @input="updateSelectedTaskConfig({ roles: parseRoles(($event.target as HTMLInputElement).value) })">
            </label>
            <label class="panel-field">
              <span class="panel-label">回写键 Write Back</span>
              <input type="text" class="panel-input mono" :value="p.selectedFormTaskConfig.value.writeBackKey" placeholder="forms.Activity_xxx" @input="updateSelectedTaskConfig({ writeBackKey: ($event.target as HTMLInputElement).value })">
            </label>
          </div>
        ...
        <div class="panel-card">
          ...
          <template v-if="p.aiConfig.value.enabled">
            <label class="panel-field">
              ...
              <input v-model="p.aiConfigAliasesText.value" type="text" class="panel-input" placeholder="例如: 开包, 登机口开包">
            </label>
            <label class="panel-field">
              <span class="panel-label">触发短语（英文逗号分隔）</span>
              <input v-model="p.aiConfigTriggerText.value" type="text" class="panel-input" placeholder="例如: 登机口需要开包">
            </label>
            ...
            <div class="panel-subblock">
              ...
              <div class="panel-grid-2">
                <label class="panel-field"><span class="panel-label">起飞前窗口 (小时)</span><input v-model.number="p.aiConfig.value.flight_matching.window_hours_before" type="number" min="0" class="panel-input"></label>
                <label class="panel-field"><span class="panel-label">起飞后窗口 (小时)</span><input v-model.number="p.aiConfig.value.flight_matching.window_hours_after" type="number" min="0" class="panel-input"></label>
                <label class="panel-field">
                ...
                <label class="panel-field"><span class="panel-label">自动匹配置信度</span><input v-model.number="p.aiConfig.value.flight_matching.min_auto_match_score" type="number" step="0.05" min="0" max="1" class="panel-input"></label>
              </div>
              <div class="check-col">
            ...
            <div class="panel-subblock">
              ...
              <label class="panel-field"><span class="panel-label">描述模板</span><textarea v-model="p.aiConfig.value.description_template" rows="2" class="panel-textarea" placeholder="登机口开包，座位号 {{seat_no}}" /></label>
              <label class="panel-field"><span class="panel-label">备注模板</span><textarea v-model="p.aiConfig.value.remarks_template" rows="2" class="panel-textarea" placeholder="座位号 {{seat_no}}" /></label>
            </div>
            ...
            <div class="panel-subblock">
              ...
              <label class="panel-field"><span class="panel-label">禁止录入字段（英文逗号分隔）</span><input v-model="p.aiConfigForbiddenText.value" type="text" class="panel-input" placeholder="gate, stand"></label>
              <label class="panel-field">
                <span class="panel-label">抽取字段定义（JSON Map）</span>
                <textarea v-model="p.aiConfigFieldsJsonText.value" rows="6" class="panel-textarea mono" placeholder='{"seat_no": {"type": "string", "label": "座位号", "required": true}}' />
                <span v-if="p.aiConfigFieldsJsonError.value" class="field-error">{{ p.aiConfigFieldsJsonError.value }}</span>
              </label>
            </div>
          </template>

          <button class="panel-btn primary" type="button" @click="p.saveAiConfig">
            保存 AI 抽取配置
          </button>
        </div>
        ...
        <div class="panel-card">
          ...
          <div class="panel-subblock">
            ...
            <div class="panel-grid-2">
              ...
              <label class="panel-field"><span class="panel-label">自动匹配置信度</span><input v-model.number="p.caseProperties.value.binding_policy.flight_match_policy.min_auto_match_score" type="number" step="0.05" min="0" max="1" class="panel-input"></label>
              <label class="panel-field"><span class="panel-label">起飞前窗口 (小时)</span><input v-model.number="p.caseProperties.value.binding_policy.flight_match_policy.time_window_hours_before" type="number" min="0" class="panel-input"></label>
              <label class="panel-field"><span class="panel-label">起飞后窗口 (小时)</span><input v-model.number="p.caseProperties.value.binding_policy.flight_match_policy.time_window_hours_after" type="number" min="0" class="panel-input"></label>
            </div>
            <div class="check-col">
          ...
          <div class="panel-subblock">
            ...
            <label class="panel-field"><span class="panel-label">摘要模板</span><textarea v-model="p.caseProperties.value.extra_info_schema.summary_template" rows="2" class="panel-textarea" placeholder="座位号 {{seat_no}}" /></label>
            <label class="panel-field">
              <span class="panel-label">字段定义（JSON Map）</span>
              <textarea v-model="p.casePropertiesFieldsJsonText.value" rows="5" class="panel-textarea mono" placeholder='{"seat_no": {"type": "string", "label": "座位号"}}' />
          ...
          <div class="panel-subblock">
            ...
            <label class="panel-field"><span class="panel-label">参与比对字段（英文逗号分隔）</span><input v-model="p.duplicatePolicyFieldsText.value" type="text" class="panel-input" placeholder="seat_no"></label>
            <label class="panel-field"><span class="panel-label">活动状态白名单（英文逗号分隔）</span><input v-model="p.duplicatePolicyStatusesText.value" type="text" class="panel-input" placeholder="INITIAL, PENDING"></label>
            <div class="check-col">
          ...
          <button class="panel-btn primary" type="button" @click="p.saveCaseProperties">
            保存业务规则属性
          </button>
        </div>
      </template>
...
.panel-header-badge {
  ...
  white-space: nowrap;
  flex-shrink: 0;
  background: var(--dh-signal-ok-soft, rgba(52, 199, 89, 0.12));
  color: var(--ws-success);
}

.panel-header-badge.editing {
...
.panel-section-title {
  ...
  margin: 0;
  padding-bottom: 8px;
  border-bottom: 2px solid var(--ws-surface-muted);
}
...
.panel-card {
  ...
  border-radius: 8px;
  padding: 14px 16px;
  box-shadow: var(--ws-shadow-sm, 0 1px 4px rgba(0, 0, 0, 0.03));
}
...
.panel-textarea:hover {
  border-color: color-mix(in srgb, var(--ws-primary) 30%, var(--admin-border));
}
...
.context-variable-chip {
  ...
  background: var(--system-blue-subtle);
  color: var(--ws-primary);
  border: 1px solid color-mix(in srgb, var(--ws-primary) 16%, transparent);
}
...
.token-btn:hover {
  border-color: color-mix(in srgb, var(--ws-primary) 40%, var(--admin-border));
}
