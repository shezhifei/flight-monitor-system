...
class ModelConfig(BaseModel):
    ...
    supports_audio_transcription: bool = Field(
        default=False, description="是否支持语音转文本"
    )
...
class ContextConfig(BaseModel):
    ...
    strategy: ContextStrategy = Field(
        default=ContextStrategy.SLIDING_WINDOW, description="上下文管理策略"
    )
    persistence: str = Field(
        default="memory", description="持久化方式: memory/redis/database"
    )
...
class AIConfig(BaseModel):
    ...
    providers: Dict[str, ProviderConfig] = Field(
        default_factory=dict, description="提供商配置"
    )

    # 能力端点配置 (v6.0 新增)
    chat_endpoint: Optional[EndpointConfig] = Field(
        default=None, description="聊天/推理端点 (DeepSeek)"
    )
    vision_endpoint: Optional[EndpointConfig] = Field(
        default=None, description="视觉理解端点 (GLM-4V)"
    )
    asr_endpoint: Optional[EndpointConfig] = Field(
        default=None, description="语音转写端点 (SenseVoice)"
    )
    tts_endpoint: Optional[EndpointConfig] = Field(
        default=None, description="语音合成端点 (CosyVoice)"
    )
    models: Dict[str, ModelConfig] = Field(default_factory=dict, description="模型配置")
    context: ContextConfig = Field(
        default_factory=ContextConfig, description="上下文配置"
    )
    conversation: ConversationConfig = Field(
        default_factory=ConversationConfig, description="对话配置"
    )
    ...
    **kwargs,
    ...
    if api_key:
        ...
        providers[provider_key] = ProviderConfig(
            api_key=api_key, default_model=model, **kwargs
        )

    config_data = {"providers": providers, **kwargs}
...
# 配置合并函数
def merge_ai_configs(
    base_config: AIConfig, override_config: Dict[str, Any]
) -> AIConfig:
    """
    合并AI配置
