...
| 2026-09-03 CI 反馈续修 | 根据 GitHub Actions `33746483712` 的失败日志继续修复：Rust/Python parity 测试不再硬编码 Windows `.venv/Scripts/python.exe`，统一解析 Windows/Unix workspace venv、`FMS_TEST_PYTHON` 显式覆盖及平台 PATH fallback；RocketMQ `BrokerConfig::default()` 在容器无法枚举本机 IP 时复用已有 loopback fallback，移除启动期 `unwrap()`；Rust Docker builder 安装 Swagger UI 构建所需的 `curl`，并停止重新制造已删除的 benchmark lib target；CI/nightly 的复制密码改为满足数据库初始化策略的显式强测试值。 |
