...
| 2026-09-04 CI 反馈四修 | Actions `33792812455` 中除完整栈 E2E 外所有 job 已通过，包括 Rust build/test/coverage/benchmark、Vue、Python、MQ Gateway、Integration、迁移、Compose 与全部守门。E2E 日志确认两项启动根因：CI 临时 Vault 私钥以 `0600` 生成，而容器降权至 `vault` 用户后无法读取只读挂载；PostgreSQL 默认 `host all all` HBA 规则不覆盖物理复制连接。CI/nightly 现显式赋予仅在临时 runner 生命周期内存在的证书文件只读权限；PostgreSQL 增加独立、幂等的初始化脚本，写入 `host replication fm_replicator all scram-sha-256`，使 standby 可使用 SCRAM 完成 `pg_basebackup`。本地 Flutter 全量 `flutter analyze` 无问题，`flutter test` 24/24 通过。 |
