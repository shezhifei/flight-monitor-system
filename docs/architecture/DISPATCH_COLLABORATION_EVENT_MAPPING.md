...
- 旧兼容时间线 `/api/v2/dispatch-orders/{order_id}/timeline`

目标不是重复描述数据库字段，而是锁定**哪些事件是协同主语义**、**哪些事件允许投影到旧视图**、以及**哪些事件绝不能倒灌回旧派工列表/旧时间线**。

## 1. 总原则

- 协同账本是追加式审计主轴，记录跨工单、群聊、通知的统一协作事实
- 统一协同视图直接消费账本事件，可同时呈现工单、消息、通知与群摘要
- 旧兼容时间线只消费“工单状态迁移语义”，不消费聊天与通知事件
- 旧派工列表 `/api/v2/dispatch-orders` 只返回工单投影，不混入协同消息、通知摘要、统一事件流

## 2. 事件类型分层

### 2.1 工单事件

这些事件属于工单业务事实，可进入统一协同视图，也允许投影为旧兼容时间线动作：

| 事件类型 | 统一协同视图 | 旧兼容时间线动作 |
| --- | --- | --- |
...
这些事件属于协作事实，只进入统一协同视图，不进入旧兼容时间线：
...
这些事件属于触达事实，只进入统一协同视图，不进入旧兼容时间线：
...
旧时间线的存在目的仅为兼容历史前端与调用方，它不是协同域的主读模型。

投影规则固定如下：

- 输入源：`DispatchCollaborationQueryService.get_legacy_order_timeline(...)`
...
- 将 `message_sent` 混入旧时间线
- 将 `notification_*` 混入旧时间线
- 在旧时间线里重新拼装群摘要、未读数、通知状态
...
这意味着新增协同摘要字段时，应优先修改统一协同查询服务，而不是回写旧派工路由或旧群聊路由。

## 5. 防回退约束

- 若新增协同事件类型，必须先决定它属于工单、群聊还是通知语义
- 若不是工单状态迁移语义，不得投影到旧兼容时间线
- 若需要新前端消费协同信息，默认进入 `/api/v2/dispatch/collaboration/...`
- 若旧接口必须兼容，仅允许增加“壳层转发或兼容投影”，不允许在旧接口上生长新的聚合编排
