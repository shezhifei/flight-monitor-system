...
| W2-2 | 侧车只做 AI | 2 | Done (2026-07-12) | 2026-07-12 | grep + 应用服务分类审计确认：sidecar 无 `INSERT/UPDATE/DELETE` 核心域真相表（`flights`/`dispatch_orders`/`todos`/`anomaly`/`domain_event_outbox` 等）的 SQL；写操作仅针对自有控制面表（`ai_*`/`agent_*`/`aip_*`/`ai_runtime_commands`）。约 110 文件为纯 AI 服务（LLM 推理/工具执行/AIP 框架），15 文件为只读查询辅助（合规），16 文件含业务写意图但均因依赖断裂而失效（仓储实现未注入/服务无参实例化 TypeError）。`infrastructure/ai/aip/action_handlers.py` 的 21 个业务写 handler 由 `test_aip_action_handlers_no_fake_success.py` 守护不假装成功；`application/services/flight/flight_command_gateway.py` 为 ADR-0002 归档死代码（待 W3-2/W3-3 清理）。SOURCE_OF_TRUTH.md 与 TECH_DEBT_DASHBOARD.md 已更新。 |
