# Orchestrate sse_reconnect_test.dart with a 30s airplane window.
...
$outFile = Join-Path $env:TEMP "sse_reconn_flutter.out.txt"
$errFile = Join-Path $env:TEMP "sse_reconn_flutter.err.txt"
...
$proc = Start-Process -FilePath $flutter -WorkingDirectory $app -ArgumentList @(
    "test", "integration_test/sse_reconnect_test.dart",
    "-d", $Device,
...
try {
    while ((Get-Date) -lt $deadline -and -not $proc.HasExited) {
        $text = Combined-Text
        if (-not $airplaneOn -and $text -match "SSE_READY_FOR_AIRPLANE") {
            Write-Host ">>> enable airplane mode (hold 30s)"
            & $adb -s $Device shell cmd connectivity airplane-mode enable
            $airplaneOn = $true
        }
        if ($airplaneOn -and -not $airplaneOff -and $text -match "SSE_READY_FOR_RESTORE") {
    ...
    if ($output -match "All tests passed") {
        Write-Host "SSE_RECONNECT_OK"
        exit 0
    }
    Write-Host "SSE_RECONNECT_FAILED"
    exit 1
}
finally {
    if (-not $airplaneOff) {
