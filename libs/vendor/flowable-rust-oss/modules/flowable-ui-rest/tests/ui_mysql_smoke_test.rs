...
//! remember-me token lifecycle both lean on identity persistence, and the token
//! row in particular is written through `insert_with_extra` with projection
//! columns. SQLite is forgiving about column types and null handling in ways
//! MySQL is not, so the login/roll/logout cycle is worth exercising on the real
//! thing.
//!
//! Requires the `mysql` feature and a reachable server; the URL comes from
//! `FLOWABLE_TEST_MYSQL_URL`, defaulting to the same value the engine's own MySQL
//! tests use. Tests **skip gracefully** when the database is down so a default
//! `cargo test` without MySQL still passes. As of this commit no local instance
//! exists, so the live path is **unrun**.
//!
//! ```powershell
...
fn mysql_config(pool_size: u32) -> ProcessEngineConfiguration {
    ProcessEngineConfiguration {
        database: DatabaseConfiguration {
            ...
            // (`sqlx_executor.rs`), so an absent instance costs ~60s once — hence
            // the cached probe below, which pays it a single time per process.
...
fn mysql_available() -> bool {
    *MYSQL_AVAILABLE.get_or_init(|| {
        match ProcessEngine::build_with_config(
            "ui-mysql-availability-probe".to_string(),
            ...
            Err(error) => {
                eprintln!(
                    "Skipping UI MySQL smoke tests: database unreachable ({error}). Set \
                     FLOWABLE_TEST_MYSQL_URL to a live instance to run them."
                );
                false
            }
        }
...
/// A served UI surface over MySQL, plus the ids this test owns.
struct TestApp {
...
/// Spawns the UI router over a MySQL-backed engine, or `None` when the database
/// is unreachable so the suite skips rather than fails.
async fn spawn(test_name: &str) -> Option<TestApp> {
    ...
    let engine = match ProcessEngine::build_with_config(
        ...
        Err(error) => {
            eprintln!("Skipping UI MySQL test '{test_name}': {error}");
            return None;
        }
    };
...
#[tokio::test]
async fn login_and_authenticated_request_work_against_mysql() {
    let Some(mut app) = spawn("ui-mysql-login").await else {
        return;
    };
    let cookie = app.login().await;

    // Reads the token row back out of MySQL and resolves the scope.
    let response = app
        .client
...
#[tokio::test]
async fn logout_deletes_the_token_row_in_mysql() {
    let Some(mut app) = spawn("ui-mysql-logout").await else {
        return;
    };
    let cookie = app.login().await;
...
#[tokio::test]
async fn token_remember_me_fields_round_trip_through_mysql() {
    let Some(mut app) = spawn("ui-mysql-token-fields").await else {
        return;
    };
    let cookie = app.login().await;
...
#[tokio::test]
async fn user_crud_round_trips_through_mysql() {
    let Some(mut app) = spawn("ui-mysql-user-crud").await else {
        return;
    };
    let cookie = app.login().await;
