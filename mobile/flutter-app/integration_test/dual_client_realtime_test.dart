...
/// 双端对拍：Web 面发聊天/通知，Native SSE ≤2s 增量；未读数对齐。
///
/// ```
/// flutter test integration_test/dual_client_realtime_test.dart -d emulator-5554
/// ```
...
const String kDeviceId = 'dual-client-realtime-device';
...
Future<void> bootNative() async {
  ...
  await initCore(
    baseUrl: kBaseUrl,
    allowCleartext: true,
    dbPath: '${dir.path}${Platform.pathSeparator}fms_dual_client_realtime.db',
...
void main() {
  ...
  testWidgets('dual-client: web send → native SSE ≤2s + unread match',
      (tester) async {
    await bootNative();
    debugPrint('DUAL_NATIVE_LOGIN_OK');

    final sse = notificationsStream().asBroadcastStream();
    await waitConnected(sse);
    debugPrint('DUAL_SSE_CONNECTED');
    ...
    debugPrint('DUAL_WEB_CHAT_SENT id=${sent['message_id']} seq=${sent['seq_no']}');
    ...
    debugPrint('DUAL_CHAT_SSE_MS=$chatMs seq=${chatEvent!.seqNo}');
    ...
    final notifData = notifResp['data'] is Map
        ...
        : notifResp;
    debugPrint('DUAL_WEB_NOTIF_SENT $notifData');
    ...
    debugPrint('DUAL_NOTIF_SSE_MS=$notifMs id=${notifEvent!.notificationId}');
    ...
    final webUnread = (webUnreadRaw['unread_count'] as num?)?.toInt() ??
        ((webUnreadRaw['data'] is Map)
            ...
            : null);
    debugPrint('DUAL_UNREAD native=$nativeUnread web=$webUnread');
    expect(webUnread, isNotNull);
    expect(nativeUnread, webUnread);

    debugPrint('DUAL_DUAL_CLIENT_OK chat_ms=$chatMs notif_ms=$notifMs');
  });

  testWidgets('remaining write paths: read/ack/checklist/handover',
      (tester) async {
    await bootNative();
