...
/// P2 SSE 重连专项：飞行模式 30s → 恢复 → 自动重连，seq 去重。
///
/// 宿主机编排见 `scripts/mobile/run_p2_sse_reconnect.ps1`。
///
/// ```
/// flutter test integration_test/p2_sse_reconnect_test.dart -d emulator-5554
/// ```
...
const String kDeviceId = 'p2-sse-reconnect-device';
...
void main() {
  ...
  testWidgets('P2 SSE reconnect after 30s airplane: no duplicate seq',
      (tester) async {
    await RustLib.init();
    ...
    await initCore(
      baseUrl: kBaseUrl,
      allowCleartext: true,
      dbPath: '${dir.path}${Platform.pathSeparator}fms_p2_sse_reconn.db',
    ...
    debugPrint('P2_SSE_FIRST_CONNECTED');

    debugPrint('P2_READY_FOR_AIRPLANE');
    await _waitConnectivity(true);
    debugPrint('P2_AIRPLANE_ON');

    await Future<void>.delayed(const Duration(seconds: 30));
    debugPrint('P2_AIRPLANE_30S_ELAPSED disconnected=$disconnected');

    debugPrint('P2_READY_FOR_RESTORE');
    await _waitConnectivity(false);
    debugPrint('P2_AIRPLANE_OFF');
    ...
    debugPrint('P2_SSE_RECONNECTED connected=$connected');
    ...
    debugPrint('P2_RECONN_SENT seq=${sent.seqNo}');
    ...
    debugPrint('P2_SEQS n=${chatSeqs.length} unique=${unique.length} $chatSeqs');
    expect(unique.length, chatSeqs.length, reason: '重连后 SSE 聊天 seq 不得重复');
    debugPrint('P2_SSE_RECONNECT_OK');
  });
}
