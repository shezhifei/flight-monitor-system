...
/// P1 验收：离线补传（applied 分支）。
///
/// 与宿主机协同的编排（logcat marker 驱动，flutter 日志 tag 为 `flutter`）：
/// 1. 测试登录后打印 `P1_READY_FOR_AIRPLANE`，然后轮询 connectivity；
/// 2. 宿主机看到 marker 后 `adb shell cmd connectivity airplane-mode enable`；
/// 3. 测试发现断网 → dispatch_action（checkin）→ 断言 Queued →
///    打印 `P1_QUEUED`；宿主机此时拉取设备 sqlite 取 client_action_id，
///    然后 `airplane-mode disable`；
/// 4. 测试发现网络恢复 → 等 3s → sync_offline_actions → 断言 applied ≥ 1，
///    打印 `P1_SYNC_RESULT`。
///
...
void main() {
  ...
  testWidgets(
    'P1-2 离线补传：断网入队→恢复→sync applied',
    (tester) async {
      await ensureRust();
      ...
      debugPrint('P1_OFFLINE_ORDER=${target.id} status=${target.status}');

      // 等宿主机开飞行模式。
      debugPrint('P1_READY_FOR_AIRPLANE');
